...
echo "Hello request:"
grpcurl -plaintext -import-path ../api/proto -d '{"cluster_id": "123abc", "client_version": "2.0"}' \
  -proto v1alpha1/server/cluster.proto localhost:80 sidero.discovery.server.Cluster.Hello

# Watch request
grpcurl -plaintext -import-path ../api/proto -d '{"cluster_id": "123abc"}' \
...
grpcurl -plaintext -import-path ../api/proto -d '{"cluster_id": "123abc", "affiliate_id": "321", "affiliate_data": "", "affiliate_endpoints": "", "ttl": "90s"}' \
  -proto v1alpha1/server/cluster.proto localhost:3000 sidero.discovery.server.Cluster.AffiliateUpdate

# AffiliateDelete request
echo "AffiliateDelete request:"
grpcurl -plaintext -import-path ../api/proto -d '{"cluster_id": "123abc", "affiliate_id": "321"}' \
